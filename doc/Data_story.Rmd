...
```{r,warning=FALSE, message=FALSE,echo=FALSE}
# loads in the processed data after it went throught Text_Processing.Rmd

load('../output/processed_lyrics.RData')
```

```{r,warning=FALSE, message=FALSE,echo=FALSE}
# Seperates all the stemmed lyrics so that each word has a row
#Then counts all the same words and then organize the rows by size
...
#Seperates all the data by decade and genres give them the name tidy_lyrics_[what it is specifically]

#All the tables count and sort all the words and a new column was add that gets the percentage of each word out of all the words in its respective table
...
#We are combining 60s and 70s data into 1 table and combining 00s and 10s data into another data
#We add a column called data to specify which years we are looking at
#we then sort them by words them reorganize it so we are grouping by decade
#Then we compare frequencies by making a jitter graph
...
```{r,warning=FALSE, message=FALSE,echo=FALSE}
#Making bar graphs with bins being equal to the number of bars in the bar graph we want
#We add a percentage column to these tables and graph a word and its percent frequency
...
```{r,warning=FALSE, message=FALSE,echo=FALSE}
#make a Pie chart for each decade by making one table hold the top 10 word and another table to hold the rest of the word
#The percentage is summed up and rounded to the nearest whole percent
#Then make the graph look pretty and graph
#found how to do it here: https://www.statmethods.net/graphs/pie.html
...
```{r,warning=FALSE, message=FALSE,echo=FALSE}
#Making bar graphs for each decade with bins being equal to the number of bars in the bar graph we want
#We add a percentage column to these tables and graph a word and its percent frequency
...
#We are combining rock data into 1 table and combining pop data into another data
#We add a column called data to specify which genre we are looking at
#we then sort them by words them reorganize it so we are grouping by decade
#Then we compare frequencies by making a jitter graph
# Code to make this graph was used from http://tzstatsads.github.io/tutorials/proj1_jiaqianyu.html
...
```{r,warning=FALSE, message=FALSE,echo=FALSE}
#Making bar graphs for pop and rock music with bins being equal to the number of bars in the bar graph we want
#We add a percentage column to these tables and graph a word and its percent frequency
...
#make a word cloud of each genre using the words and frequency numbers
...
#Making bar graphs for all genres music with bins being 1 more than to the number of bars in the bar graph we want
#We add a percentage column to these tables and graph a word and its percent frequency
...
#make a Pie chart for each genre by making one table hold the top 10 word and another table to hold the rest of the word
#The percentage is summed up and rounded to the nearest whole percent
#Then make the graph look pretty and graph
#found how to do it here: https://www.statmethods.net/graphs/pie.html
